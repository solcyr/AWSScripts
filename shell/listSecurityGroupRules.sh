...
#Ingress
aws $AWS_OPTS ec2 describe-security-groups --region $region --filters Name=vpc-id,Values=$vpcId --query SecurityGroups[].IpPermissions > $TMPFILE
...
#Egress
aws $AWS_OPTS ec2 describe-security-groups --region $region --filters Name=vpc-id,Values=$vpcId --query SecurityGroups[].IpPermissionsEgress > $TMPFILE
